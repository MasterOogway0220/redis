...
- [x] Fuzz targets for `kvlite-resp` decode and round trip
...
- [ ] `cargo semver-checks` — the CI job exists but has no baseline to compare against until the first release, so it is `continue-on-error` for now
